...
<template>
    <div v-show="showmodal" class="add-banner-modal">
        <ui-modal :show.sync="showmodal">
            ...
            <form slot="body" class="form-horizontal" role="form" v-el:form>
                <div class="form-group">
                    <label for="add-banner-title" class="col-sm-3 control-label">Title : </label>
                    <div class="col-sm-8">
                        <input id="add-banner-title" type="text" name="title" class="form-control" required/>
                    </div>
                </div>
                <div class="form-group upload-pic">
                    <label for="add-banner-img" class="col-sm-3 control-label">Banner : </label>
                    <div class="col-sm-8">
                        <img-pre-wrap :items=images :len=1 v-ref:banner></img-pre-wrap>
                    </div>
                </div>
                <div class="form-group">
                    <label for="add-banner-title" class="col-sm-3 control-label">URL : </label>
                    <div class="col-sm-8">
                        <input id="add-banner-title" type="text" name="path" class="form-control" required/>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group" style="text-align: center">
                    <button type="submit" class="btn add-banner-btn" @click.top.prevent="addBanner">FINISH</button>
                </div>
            </form>
    ...
    export default {
        ...
        methods: {
            addBanner: function () {
                ...
                post(this, '/console/api/banner/add', formData, function (data, status) {
                    if (data.sta === 0) {
                        self.$els.form.reset();
